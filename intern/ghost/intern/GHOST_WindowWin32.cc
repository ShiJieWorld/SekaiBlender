...
constexpr char kSekaiBlenderWinAppID[] = "sekaiblender.1.0";
constexpr char kSekaiBlenderWinAppName[] = "SekaiBlender 1.0";
...
void GHOST_WindowWin32::registerWindowAppUserModelProperties()
{
  ...
  /* Find the current executable, and see if it's SekaiBlender.exe if not bail out. */
  GetModuleFileName(0, blender_path, sizeof(blender_path));
  char *blender_app = strstr(blender_path, "SekaiBlender.exe");
  if (!blender_app) {
    return;
  }
  ...
  strcpy(blender_app, "SekaiBlender-launcher.exe");
  wsprintfW(shell_command, L"\"%S\"", blender_path);
  UTF16_ENCODE(kSekaiBlenderWinAppID);
  UTF16_ENCODE(kSekaiBlenderWinAppName);
  PROPVARIANT propvar;
  hr = InitPropVariantFromString(kSekaiBlenderWinAppID_16, &propvar);
  ...
  hr = InitPropVariantFromString(kSekaiBlenderWinAppName_16, &propvar);
  hr = pstore->SetValue(PKEY_AppUserModel_RelaunchDisplayNameResource, propvar);
  pstore->Release();
  UTF16_UN_ENCODE(kSekaiBlenderWinAppName);
  UTF16_UN_ENCODE(kSekaiBlenderWinAppID);
}
