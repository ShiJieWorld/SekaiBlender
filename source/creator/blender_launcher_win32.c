...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
  ...
  /* Get the path to the currently running executable (`SekaiBlender-launcher.exe`). */
  ...
  /* Remove the filename (SekaiBlender-launcher.exe) from path. */
  if (PathCchRemoveFileSpec(path, MAX_PATH) != S_OK) {
    return -1;
  }

  /* Add SekaiBlender.exe to path, resulting in the full path to the application executable. */
  if (PathCchCombine(path, MAX_PATH, path, L"SekaiBlender.exe") != S_OK) {
    return -1;
  }
  ...
  if (success) {
    /* If SekaiBlender-launcher is called with background command line flag or launched from steam,
