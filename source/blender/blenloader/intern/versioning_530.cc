...
/* Define versioning helpers from `DNA_genfile.h`. */
...
void blo_do_versions_530(FileData *fd, Library * /*lib*/, Main *bmain)
{
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 503, 11)) {
    if (!DNA_struct_member_exists(fd->filesdna, "SceneEEVEE", "float", "fsr_sharpness")) {
      const SceneEEVEE defaults;
      for (Scene &scene : bmain->scenes) {
        scene.eevee.fsr_sharpness = defaults.fsr_sharpness;
      }
    }
    if (!DNA_struct_member_exists(fd->filesdna, "SceneEEVEE", "int", "fsr_quality")) {
      const SceneEEVEE defaults;
      for (Scene &scene : bmain->scenes) {
        scene.eevee.fsr_quality = defaults.fsr_quality;
      }
    }
  }
