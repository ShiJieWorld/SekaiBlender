...
TEST_F(ArmatureDeformTest, PMXSDEFMeshDeform)
{
  Object *ob_arm = this->create_test_armature_object();
  Object *ob_target = this->create_test_mesh_object();
  Mesh *mesh = id_cast<Mesh *>(ob_target->data);

  bPoseChannel *pchan1 = BKE_pose_channel_find_name(ob_arm->pose, "Bone1");
  unit_qt(pchan1->quat);
  pchan1->quat[0] = 0.70710677f;
  pchan1->quat[3] = 0.70710677f;
  zero_v3(pchan1->loc);
  update_pose_matrices({pchan1, pchan1->bone_get(*id_cast<bArmature *>(ob_arm->data))});
  ...
  weight_types.span.fill(0);
  sdef_c.span.fill(float3(0.0f));
  sdef_r0.span.fill(float3(0.0f));
  sdef_r1.span.fill(float3(0.0f));
  sdef_bone0.span.fill(-1);
  sdef_bone1.span.fill(-1);
  weight_types.span[4] = 3;
  /* Non-degenerate C/R0/R1 data verifies the PMX correction points as well as rotation. */
  sdef_c.span[4] = float3(0.0f, 0.0f, 1.0f);
  sdef_r0.span[4] = float3(2.0f, 0.0f, 1.0f);
  sdef_r1.span[4] = float3(0.0f, 2.0f, 1.0f);
  sdef_bone0.span[4] = 0;
  sdef_bone1.span[4] = 1;
  weight_types.finish();
  sdef_c.finish();
  ...
  const float3 original_sdef_position = positions[4];
  BKE_armature_deform_coords_with_mesh(*ob_arm,
                                       *ob_target,
                                       positions,
                                       std::nullopt,
                                       std::nullopt,
                                       ARM_DEF_VGROUP | ARM_DEF_QUATERNION,
                                       "",
                                       mesh);
