    ...
    TreeElement &parent_ten)
{
  for (LayerCollection &lc : layer_collections) {
    ...
    if (exclude && ((space_outliner_.show_restrict_flags & SO_RESTRICT_ENABLE) == 0)) {
       ...
       * [世界的歌] PMX import marks its implementation collections as collapsed so
       * importing a material-split model does not flood the Outliner with every mesh object. */
      TreeStoreElem *tselem = TREESTORE(ten);
      const IDProperty *system_props = IDP_ID_system_properties_get(id);
      const bool pmx_collapsed = system_props &&
                                 IDP_group_lookup_int(*system_props, "pmx_outliner_collapsed").value_or(0) == 1;
      if (!tselem->used) {
        if (pmx_collapsed || !ID_IS_EDITABLE(id) || ID_IS_OVERRIDE_LIBRARY(id)) {
          tselem->flag |= TSE_CLOSED;
        }
        else {
          tselem->flag &= ~TSE_CLOSED;
