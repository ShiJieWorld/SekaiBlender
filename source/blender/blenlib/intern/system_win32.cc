...
/**
 * Retrieve the path to "SekaiBlender-launcher.exe" if it exists; otherwise, return the current
 * executable path.
 */
static bool bli_executable_path_get(LPWSTR path, DWORD size)
{
  ...
  /* Replace the filename "SekaiBlender.exe" with "SekaiBlender-launcher.exe". */
  if (!PathRemoveFileSpecW(executable_path)) {
    /* Failed to remove the file spec. Use the original path. */
    return true;
  }
  if (!PathAppendW(executable_path, L"SekaiBlender-launcher.exe")) {
    /* Failed to append the new filename. Use the original path. */
    return true;
  }

  /* Check if "SekaiBlender-launcher.exe" exists at this path. */
  DWORD attributes = GetFileAttributesW(executable_path);
  if (attributes == INVALID_FILE_ATTRIBUTES || (attributes & FILE_ATTRIBUTE_DIRECTORY)) {
    /* "SekaiBlender-launcher.exe" does not exist. Use the original executable path. */
    return true;
  }
