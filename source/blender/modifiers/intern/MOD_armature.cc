...
static void update_depsgraph(ModifierData *md, const ModifierUpdateDepsgraphContext *ctx)
{
  ...
  if (amd->object != nullptr) {
    ...
    /* Native MMD CCD runs at POSE_DONE, after the individual BONE_DONE nodes.
     * Ensure deformation cannot consume the pre-CCD bone caches. */
    if (amd->object->id.system_properties != nullptr &&
        IDP_GetPropertyFromGroup_null(amd->object->id.system_properties,
                                      "mmd_pmx_bone_ik_definition") != nullptr)
    {
      DEG_add_object_relation(
          ctx->node, amd->object, DEG_OB_COMP_EVAL_POSE, "MMD CCD Armature Modifier");
    }
