...
static float fsr_render_scale_get(const eSceneEEVEE_FSRQuality quality)
{
  switch (quality) {
    case SCE_EEVEE_FSR_QUALITY_ULTRA:
      return 1.0f / 1.3f;
    case SCE_EEVEE_FSR_QUALITY_QUALITY:
      return 1.0f / 1.5f;
    case SCE_EEVEE_FSR_QUALITY_BALANCED:
      return 1.0f / 1.7f;
    case SCE_EEVEE_FSR_QUALITY_PERFORMANCE:
      return 0.5f;
  }
  return 0.5f;
}
...
void Film::init(const int2 &extent, const rcti *output_rect)
{
  ...
  }
  {
    ...
    render_scale_ = 1.0f;
    const bool use_fsr = inst_.is_viewport() ?
                             (scene.eevee.flag & SCE_EEVEE_FSR_VIEWPORT) != 0 :
                             (scene.eevee.flag & SCE_EEVEE_FSR_RENDER) != 0;
    const bool fsr_changed = assign_if_different(fsr_enabled_, use_fsr);
    if (fsr_changed) {
      inst_.sampling.reset();
      if (!fsr_enabled_) {
        /* Film instances survive across frames and scene setting changes. Do
         * not retain full-resolution FSR textures after the feature is off. */
        fsr_context_.reset();
        combined_final_tx_ = nullptr;
        fsr_active_ = false;
      }
    }
    if (assign_if_different(fsr_sharpness_, scene.eevee.fsr_sharpness) && fsr_enabled_) {
      inst_.sampling.reset();
    }
    if (assign_if_different(fsr_quality_, int(scene.eevee.fsr_quality)) && fsr_enabled_) {
      inst_.sampling.reset();
    }
    if (fsr_enabled_) {
      render_scale_ = fsr_render_scale_get(scene.eevee.fsr_quality);
      data_.scaling_factor = scene.eevee.fsr_quality == SCE_EEVEE_FSR_QUALITY_PERFORMANCE ? 2 : 0;
    }
    else if (inst_.is_viewport()) {
      data_.scaling_factor = BKE_render_preview_pixel_size(&inst_.scene->r);
      render_scale_ = 1.0f / float(data_.scaling_factor);
    }
    /* Sharpen the LODs (1.5x) to avoid TAA filtering causing over-blur (see #122941). */
    data_.texture_lod_bias = render_scale_ / 1.5f;
  }
  {
    rcti fallback_rect;
    ...
    if (data_.scaling_factor == 0) {
      data_.render_extent = int2(math::ceil(float2(data_.extent) * render_scale_));
    }
    else {
...
void Film::init_pass(PassSimple &pass, gpu::Shader *sh)
{
  ...
  pass.shader_set(sh);
  pass.push_constant("combined_is_upscaled", &fsr_active_);
...
void Film::accumulate(View &view, gpu::Texture *combined_final_tx)
{
  ...
  fsr_active_ = false;
  if (fsr_enabled_) {
    const int2 input_extent = data_.render_extent - int2(data_.overscan * 2);
    gpu::Texture *fsr_output = fsr_context_.upsample(combined_final_tx,
                                                     data_.overscan,
                                                     data_.overscan,
                                                     input_extent.x,
                                                     input_extent.y,
                                                     ...
                                                     data_.extent.y,
                                                     fsr_sharpness_);
    if (fsr_output != nullptr) {
      combined_final_tx = fsr_output;
      fsr_active_ = true;
    }
    else {
      inst_.info_append_i18n("FSR upscaling failed; using native EEVEE reconstruction.");
    }
  }
