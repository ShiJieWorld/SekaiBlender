...
/** Add headroom and align to complete atlas layers without exceeding packed page addressing. */
int shadow_page_count_with_headroom(int current_page_count, int required_page_count);
...
class ShadowModule {
 ...
 private:
  ...
  /** User setting is the initial allocation; runtime statistics can grow this value. */
  int shadow_pool_size_mb_ = -1;
  int shadow_page_len_adaptive_ = 0;
  /** Global switch. */
  bool enabled_ = true;

  bool shadow_atlas_ensure();
  void shadow_page_allocator_reset();
  void shadow_statistics_reset();
  bool shadow_page_capacity_grow(int required_page_count, bool reset_allocator);
 ...
 private:
  void remove_unused();
  bool shadow_update_finished(int &loop_count, int &growth_retry_count);
