...
int shadow_page_count_with_headroom(const int current_page_count, const int required_page_count)
{
  if (required_page_count <= current_page_count) {
    return current_page_count;
  }
  const int page_count_with_headroom = required_page_count + (required_page_count + 3) / 4;
  const int page_count_aligned =
      ((page_count_with_headroom + SHADOW_PAGE_PER_LAYER - 1) / SHADOW_PAGE_PER_LAYER) *
      SHADOW_PAGE_PER_LAYER;
  return min_ii(page_count_aligned, SHADOW_MAX_PAGE);
}
...
bool ShadowModule::shadow_atlas_ensure()
{
  ...
  const eGPUTextureUsage tex_usage = GPU_TEXTURE_USAGE_SHADER_READ |
                                     GPU_TEXTURE_USAGE_SHADER_WRITE | GPU_TEXTURE_USAGE_ATOMIC;
  if (atlas_tx_.is_valid() && atlas_tx_.width() == atlas_extent.x &&
      atlas_tx_.height() == atlas_extent.y &&
      GPU_texture_layer_count(atlas_tx_) == atlas_layers)
  {
    return true;
  }

  Texture replacement("shadow_atlas_tx_", atlas_type, tex_usage, atlas_extent, atlas_layers);
  if (!replacement.is_valid()) {
    return false;
  }
  replacement.filter_mode(false);
  Texture::swap(atlas_tx_, replacement);
  do_full_update_ = true;
  return true;
}

void ShadowModule::shadow_page_allocator_reset()
{
  for (uint i : IndexRange(shadow_page_len_)) {
    const uint3 page = {i % SHADOW_PAGE_PER_ROW,
...
void ShadowModule::shadow_statistics_reset()
{
  for (int i = 0; i < statistics_buf_.size(); i++) {
    UNUSED_VARS(i);
    statistics_buf_.current().clear_to_zero();
...
bool ShadowModule::shadow_page_capacity_grow(const int required_page_count,
                                             const bool reset_allocator)
{
  const int previous_page_len = shadow_page_len_;
  const int grown_page_len = shadow_page_count_with_headroom(previous_page_len,
                                                             required_page_count);
  if (grown_page_len <= previous_page_len) {
    return false;
  }

  shadow_page_len_ = grown_page_len;
  if (!shadow_atlas_ensure()) {
    shadow_page_len_ = previous_page_len;
    shadow_atlas_ensure();
    return false;
  }

  shadow_page_len_adaptive_ = shadow_page_len_;
  if (reset_allocator) {
    shadow_page_allocator_reset();
  }

  const size_t page_byte_size = square_i(shadow_page_size_) * sizeof(int);
  const size_t previous_pool_mb = size_t(previous_page_len) * page_byte_size / square_i(1024);
  const size_t grown_pool_mb = size_t(shadow_page_len_) * page_byte_size / square_i(1024);
  inst_.info_append_i18n(
      "Shadow buffer automatically expanded from {} MB to {} MB ({} pages required).",
      previous_pool_mb,
      grown_pool_mb,
      required_page_count);
  return true;
...
void ShadowModule::init()
{
  ...
  const size_t pool_byte_size = enabled_ ? size_t(scene.eevee.shadow_pool_size) * square_i(1024) :
                                           1;
  const size_t page_byte_size = square_i(shadow_page_size_) * sizeof(int);
  const int configured_page_len = int(divide_ceil_ul(pool_byte_size, page_byte_size));
  const bool pool_configuration_changed = assign_if_different(shadow_pool_size_mb_,
                                                               scene.eevee.shadow_pool_size) ||
                                          !enabled_;
  if (pool_configuration_changed) {
    shadow_page_len_adaptive_ = configured_page_len;
    shadow_statistics_reset();
  }
  const int effective_page_len = min_ii(max_ii(configured_page_len, shadow_page_len_adaptive_),
                                        SHADOW_MAX_PAGE);
  do_full_update_ |= assign_if_different(shadow_page_len_, effective_page_len);
  ...
  if (inst_.is_viewport()) {
    ...
    if (!pool_configuration_changed && stats.page_used_count > shadow_page_len_ && enabled_ &&
        !shadow_page_capacity_grow(stats.page_used_count, false))
    {
      inst_.info_append_i18n(
          "Error: Shadow buffer reached its 2 GB addressable limit; shadows may be missing. "
          "Reduce Shadow Resolution. ({} / {} pages)",
...
bool ShadowModule::shadow_update_finished(int &loop_count, int &growth_retry_count)
{
  ...
  /* Page pressure is independent of the number of tile-maps and can exceed the physical pool even
   * when all views fit in a single update. */
  const bool check_page_pressure = max_updated_view_count > 0;
  if (!check_page_pressure && max_updated_view_count <= SHADOW_VIEW_MAX * loop_count) {
  ...
  if (loop_count == 2) {
    ...
    if (stats.page_used_count > shadow_page_len_) {
      if (growth_retry_count < 4 && shadow_page_capacity_grow(stats.page_used_count, true)) {
        growth_retry_count++;
        loop_count = 0;
        needed_views_ = 0;
        return false;
...
void ShadowModule::render(View &view, int2 extent)
{
  ...
  } while (!shadow_update_finished(loop_count, growth_retry_count));
