...
struct Film {
  ...
  [[push_constant]] bool combined_is_upscaled;
  ...
  FilmSample sample_get(int sample_n, int2 texel_film)
  {
    ...
    if (scaling_factor > 1) {
      ...
      /* Map final film pixels to the arbitrary internal FSR render resolution. */
      int2 base_render_extent = uni.uniform_buf.film.render_extent -
                                int2(uni.uniform_buf.film.overscan * 2);
      float2 render_to_film = float2(uni.uniform_buf.film.extent) /
                              float2(base_render_extent);
      float2 render_coord = (float2(texel_film) + 0.5f) / render_to_film +
                            uni.uniform_buf.film.subpixel_offset -
                            0.5f + float(uni.uniform_buf.film.overscan);
      int2 render_base = int2(floor(render_coord));
      film_sample.texel += render_base;

      float2 sample_coord = (float2(film_sample.texel -
                                    int2(uni.uniform_buf.film.overscan)) +
                             0.5f - uni.uniform_buf.film.subpixel_offset) *
                            render_to_film;
      float2 film_coord = float2(texel_film) + 0.5f;
      float2 sample_delta = sample_coord - film_coord;
      film_sample.weight = film_filter_weight(
          uni.uniform_buf.film.filter_radius, length_squared(sample_delta));
      film_sample.weight = max(film_sample.weight, 1e-8f);
    }
  ...
  void combined_neighbor_boundbox(int2 texel, float4 &min_c, float4 &max_c)
  {
    ...
    max_c = float4(-1e16f);
    for (int i = 0; i < 5; i++) {
      int2 sample_texel = clamp(
          texel + plus_offsets[i], int2(0), textureSize(combined_tx, 0).xy - 1);
      float4 color = texelfetch_as_YCoCg_opacity(combined_tx, sample_texel);
    ...
    for (int i = 0; i < 4; i++) {
      int2 sample_texel = clamp(texel + corners[i], int2(0), textureSize(combined_tx, 0).xy - 1);
      float4 color = texelfetch_as_YCoCg_opacity(combined_tx, sample_texel);
                      ...
                      float4 &display)
  {
    ...
    if (use_reprojection) {
      ...
      float2 motion = pixel_history_motion_vector(motion_texel);
      ...
      combined_neighbor_boundbox(color_texel, min_color, max_color);
  ...
  void process_render_sample(int2 texel_film, float4 &out_color, float &out_depth)
  {
    ...
    if (combined_id != -1) {
      ...
      FilmSample color_src;
      FilmSample motion_src = sample_get(0, texel_film);
      if (combined_is_upscaled) {
        color_src.texel = texel_film;
        color_src.weight = 1.0f;
        sample_accum_combined(color_src, combined_accum, weight_accum);
      }
      else {
        for (int i = samples_len - 1; i >= 0; i--) {
          color_src = sample_get(i, texel_film);
          sample_accum_combined(color_src, combined_accum, weight_accum);
        }
      }
      store_combined(dst,
