...
void ShadingView::update_view()
{
  ...
  if (film.use_scaled_resolution_get()) {
    ...
    const int overscan_pixels = film.render_overscan_get();
    const int2 base_render_extent = render_extent - int2(overscan_pixels * 2);
    /* Pixel size in the actual internal render target. */
    const float2 pixel_size = render_size / float2(base_render_extent);
