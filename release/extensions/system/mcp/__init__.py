...
class _BlenderMCPPreferences(bpy.types.AddonPreferences):  # type: ignore[misc]
    ...
    use_autostart: BoolProperty(  # type: ignore[valid-type]
        name="Auto Start",
        description=(
            "Automatically start the Local AI Bridge when SekaiBlender starts.\n"
            "Without this, start it from Preferences after enabling Online Access.\n"
            "(ignored in background mode)"
        ),
        # SekaiBlender ships this official Lab add-on but keeps listening off
        # until the user starts it from Preferences (then they may enable this).
        default=False,
    )
    ...
    def draw(self, context: bpy.types.Context) -> None:
        ...
        if mcp_to_blender_server.is_running():
            ...
            layout.label(text="Local AI Bridge is running", icon="CHECKMARK")
        else:
            layout.operator("blmcp.server_start", icon="PLAY")
            layout.label(text="Local AI Bridge is stopped", icon="X")
...
class _BLMCP_OT_server_start(bpy.types.Operator):  # type: ignore[misc]
    ...
    bl_label = "Start Local AI Bridge"
    bl_description = "Listen on localhost so AI agents can drive this open scene"
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        ...
        if bpy.app.background:
            self.report({"ERROR"}, "Use `--command blender_mcp` to start the Local AI Bridge in background mode")
            return {"CANCELLED"}
        ...
        bpy.app.timers.register(
            ...
            persistent=True)
        self.report({"INFO"}, "Local AI Bridge started on {:s}:{:d}".format(prefs.host, prefs.port))
        return {"FINISHED"}


class _BLMCP_OT_server_stop(bpy.types.Operator):  # type: ignore[misc]
    bl_idname = "blmcp.server_stop"
    bl_label = "Stop Local AI Bridge"
    bl_description = "Stop listening on localhost"
    ...
    def execute(self, context: bpy.types.Context) -> set[str]:
        ...
        self.report({"INFO"}, "Local AI Bridge stopped")
        return {"FINISHED"}
